...
2. 词类 + 汉语释义列：
    1. 如果一个单词有多个词类(性)：
        1. (1) 根据词性的使用频率和重要程度，从高到底排序；
        2. (2) 一个词性的汉语翻译我希望只给出 1 到 2 个最重要的即可。(注：如果两个汉语释义相似，我会只保留最核心的一个汉语翻译，以确保简洁性和易学性。)
        3. (3) 比较不重要的词性，请给出 1 个最相关的汉语翻译，这样不至于次要的词类完全不知道。
        4. (4) 所有词性放在一行，不同词类之间使用分号分隔。
