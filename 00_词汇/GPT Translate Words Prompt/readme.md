...
*Update: 2024.07.18, ChatGPT 更新总结：*
...
#### 2. 词性 + 汉语释义
##### 2.1 如果一个单词有多个词性：
- 多词性排序：根据词性的使用频率和重要程度从高到低排序。
- 词性分隔：所有词类放在一行，不同词类之间使用 `<br>` 分隔。

##### 2.2 汉语释义的数量
- **最重要词性**：根据汉语释义的重要程度和相关性，提供1至3个重要的汉语释义。
- **次要词性**：通常提供1个最相关的汉语释义，如果有两个重要和常用的汉语翻译，可以保留两个，但不超过两个。

##### 2.3 添加常用说明
- 对于一些常见的用法，在释义后添加注释，例如：`n. 祝贺; 恭喜（通常用作复数）`

#### 3. 特殊要求
- 对于某些单词，如果汉语释义包含括号内容（如解释特定用法），括号使用英文格式。
- 当单词的词性大于1个时，英文句子之前才会添加词性简写。
- 示例列先不需要添加，等校对完单词翻译列表后再单独添加示例列。
